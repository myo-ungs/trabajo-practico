...
if __name__ == "__main__":
    ...
    base_output = Path("parte7") / "OUTPUT"

    grupos = ["a", "b"]
    instancias = ["instance_0001.txt", "instance_0002.txt", "instance_0003.txt"]

    partes = {
        "Parte 6": base_output / "output_modelo_parte6",
        "Parte 7": base_output / "output_modelo_parte7",
    }

    print("=" * 70)
    print("VERIFICACIÓN DE SOLUCIONES (A y B)")
    print("=" * 70)

    for grupo in grupos:
        print(f"\nGrupo de instancias {grupo.upper()}")
        print("-" * 70)
        ...
        for instancia in instancias:
            ...
            for nombre_parte, carpeta_output in partes.items():
                path_out = carpeta_output / f"{grupo}_{instancia.replace('.txt', '.out')}"

                print(f"   ▶ {nombre_parte}")
